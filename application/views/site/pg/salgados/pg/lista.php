<div class="page-header">
    <h1>Todos os Salgados</h1>
</div>
<div class="result-group">
    <div class="resultado_lancamento" >
        <a href="<?php echo base_url('salgados/item') ?>"><img class="img img-polaroid img-rounded" data-src="holder.js/360x216/auto"/></a>
        <h5 id="titulo_pesquisa">Titulo da Imagem</h5>
        <span>Conteudo referente a imagem mostrada acima. Especificações do produto em questão.</span>    
        <div>
            <a href="<?php echo base_url('salgados/item') ?>" class="btn btn-warning btn-mini">Ver detalhe [+]</a>
            <input type="button" class="btn btn-success btn-mini" value="Indicar para um amigo >>" />
        </div>
    </div>
    <div class="resultado_lancamento" >
        <a href="<?php echo base_url('salgados/item') ?>"><img class="img img-polaroid img-rounded" data-src="holder.js/360x216/auto"/></a>
        <h5 id="titulo_pesquisa">Titulo da Imagem</h5>
        <span>Conteudo referente a imagem mostrada acima. Especificações do produto em questão.</span>    
        <div>
            <a href="<?php echo base_url('salgados/item') ?>" class="btn btn-warning btn-mini">Ver detalhe [+]</a>
            <input type="button" class="btn btn-success btn-mini" value="Indicar para um amigo >>" />
        </div>
    </div>
    <div class="resultado_lancamento" >
        <a href="<?php echo base_url('salgados/item') ?>"><img class="img img-polaroid img-rounded" data-src="holder.js/360x216/auto"/></a>
        <h5 id="titulo_pesquisa">Titulo da Imagem</h5>
        <span>Conteudo referente a imagem mostrada acima. Especificações do produto em questão.</span>    
        <div>
            <a href="<?php echo base_url('salgados/item') ?>" class="btn btn-warning btn-mini">Ver detalhe [+]</a>
            <input type="button" class="btn btn-success btn-mini" value="Indicar para um amigo >>" />
        </div>
    </div>
</div>

<div class="pagination" id="paginacao" align="center">
    <ul>
        <li><a href="#"><<</a></li>
        <li><a href="#">1</a></li>
        <li><a href="#">2</a></li>
        <li><a href="#">3</a></li>
        <li><a href="#">4</a></li>
        <li><a href="#">5</a></li>
        <li><a href="#">>></a></li>
    </ul>
</div>